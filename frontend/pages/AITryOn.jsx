...
import { Camera, Sparkles, AlertCircle, CheckCircle2, ArrowRight, ArrowLeft, Star, Clock, Briefcase, Crown, Scissors } from 'lucide-react';
...
const AITryOn = () => {
    const [state, setState] = useState(() => {
        return sessionStorage.getItem('aiState') || 'initial';
    });
    ...
    return (<div className="bg-gray-900 min-h-screen py-12 px-4">
      <div className="container mx-auto max-w-6xl">
        ...
        {state === 'initial' && (<div className="max-w-xl mx-auto bg-gray-50 rounded-xl border-2 border-dashed border-gray-300 p-10 text-center hover:border-amber-500 transition-colors">
            {userImage ? (<div className="space-y-8">
                ...
                <div className="flex flex-col gap-3">
                  <button onClick={proceedToPreferences} className="w-full py-4 bg-amber-600 text-white font-bold uppercase tracking-widest rounded-sm hover:bg-amber-500 shadow-lg transform hover:-translate-y-1 transition-all flex items-center justify-center gap-2">
                    Continue <ArrowRight size={18}/>
                  </button>
                  <button onClick={() => setUserImage(null)} className="text-gray-500 text-sm font-bold hover:text-gray-900">
                    Change Photo
                  </button>
                </div>
                ...
                <button onClick={() => fileInputRef.current?.click()} className="px-8 py-3 bg-gray-900 text-white font-bold uppercase tracking-wider rounded-sm hover:bg-gray-800 transition-all">
                  Select Image
                </button>
              </div>)}
          </div>)}
        ...
        {/* PREFERENCES STATE */}
        {state === 'preferences' && (<div className="max-w-2xl mx-auto space-y-8">
            ...
            ].map(({ value, label, icon: Icon }) => (<button key={value} onClick={() => updatePreference('occasion', value)} className={`p-4 rounded-lg border-2 text-left transition-all ${preferences.occasion === value
            ...
            ].map(({ value, label, desc }) => (<button key={value} onClick={() => updatePreference('maintenance', value)} className={`p-4 rounded-lg border-2 text-center transition-all ${preferences.maintenance === value
                    ? 'border-amber-500 bg-amber-50 text-amber-700'
                    : 'border-gray-200 hover:border-gray-300 text-gray-600'}`}>
            ...
            <div className="flex gap-4">
              <button onClick={() => setState('initial')} className="flex-1 py-3 border border-gray-300 text-white font-bold uppercase tracking-wider rounded-sm hover:bg-gray-50 transition-all flex items-center justify-center gap-2">
                <ArrowLeft size={18}/> Back
              </button>
              <button onClick={startAnalysis} className="flex-1 py-3 bg-amber-600 text-white font-bold uppercase tracking-wider rounded-sm hover:bg-amber-500 transition-all flex items-center justify-center gap-2">
        ...
        {/* ANALYZING STATE */}
        {state === 'analyzing' && (<div className="max-w-xl mx-auto text-center py-20 space-y-8">
            <div className="relative w-32 h-32 mx-auto">
              <div className="absolute inset-0 border-4 border-gray-200 rounded-full"></div>
              <div className="absolute inset-0 border-4 border-amber-500 rounded-full border-t-transparent animate-spin"></div>
              <img src={userImage} className="absolute inset-2 rounded-full object-cover w-[calc(100%-16px)] h-[calc(100%-16px)] opacity-50" alt="Analyzing"/>
            </div>
            <div>
        ...
        {/* RESULTS STATE */}
        {state === 'results' && userImage && faceAnalysis && (<div className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              {/* Right: Best Combo */}
              {recommendations.find(r => r.type === 'combo') && (<div className="bg-white rounded-lg overflow-hidden shadow-lg border-2 border-purple-200">
                  ...
                  {(() => {
                    ...
                    return (<>
                        <div className="relative aspect-[3/4] bg-gray-100 overflow-hidden">
                          ...
                          {combo.beardImage && (<img src={combo.beardImage} alt="Beard" className="w-full h-1/2 object-cover absolute bottom-0 left-0"/>)}
                        ...
                        <div className="p-4">
                          ...
                          <button onClick={() => bookStyle(combo)} className="w-full py-2 px-3 bg-purple-600 text-white rounded-sm text-sm font-bold hover:bg-purple-500 transition-all">
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-3">
                <button onClick={reset} className="px-4 py-2 border border-gray-300 rounded-sm text-sm font-bold text-gray-600 hover:bg-gray-100 transition-colors">
                  Start Over
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {recommendations.filter(r => r.type === 'hair').map((style) => (<div key={style.id} className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-all border border-gray-100 group">
                    <div className="relative aspect-[3/4] bg-gray-100 overflow-hidden">
                    ...
                    <div className="p-4">
                      ...
                      <div className="space-y-2">
                        <button onClick={() => bookStyle(style)} className="w-full py-2 px-3 bg-amber-600 text-white rounded-sm text-sm font-bold hover:bg-amber-500 transition-all">
                          Book This Style
                        </button>
                      </div>
                    </div>
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {recommendations.filter(r => r.type === 'beard').map((style) => (<div key={style.id} className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-all border border-gray-100 group">
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex flex-wrap gap-1 mb-3">
                        {style.tags.slice(0, 3).map(tag => (<span key={tag} className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full capitalize">
                            {tag}
                          </span>))}
                      </div>
                      <div className="flex justify-between items-center text-xs text-gray-500 mb-3">
