...
const Booking = () => {
    ...
    if (!user) {
        return (<div className="min-h-[60vh] flex flex-col items-center justify-center px-4 text-center bg-gray-800">
    ...
    useEffect(() => {
        const fetchBarbers = async () => {
            try {
                ...
                const formattedBarbers = data.map((b) => ({
                    ...b,
                    id: b.id.toString()
                }));
    ...
    const handleProceedToCheckout = async () => {
        ...
        try {
            ...
            if (response.message.includes('available') && !response.message.includes('Sorry')) {
                ...
                setModalContent(<div className="space-y-4">
            <p className="text-lg">{response.message}</p>
            {response.availableBarbers && response.availableBarbers.length > 0 && (<div>
                <p className="text-sm text-gray-400 mb-2">Available barbers at this time:</p>
                <div className="flex flex-wrap gap-2">
                  {response.availableBarbers.map((b) => (<button key={b.id} onClick={() => {
                ...
                setModalActions(<button onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-gray-700 text-white rounded hover:bg-gray-600 transition-colors">
            Close
          </button>);
                setIsModalOpen(true);
            }
    ...
    return (<div className="min-h-screen bg-gray-900 text-gray-100 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Progress Steps */}
        {step < 3 && (<div className="flex justify-center mb-12">
        ...
        {step === 1 && (<div className="grid md:grid-cols-3 gap-8">
            <div className="md:col-span-2 space-y-4">
              <h2 className="text-2xl font-serif font-bold text-white mb-4">Review Services</h2>
              {cart.map((service) => (<div key={service.id} className="bg-gray-800 p-6 rounded-sm shadow-sm flex items-center justify-between border border-gray-700">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {service.note && (<p className="text-sm text-amber-500 italic mt-1">{service.note}</p>)}
                    </div>
                  </div>
                  <div className="flex items-center gap-8">
                    <span className="font-bold text-white text-lg">${service.price}</span>
                    <button onClick={() => removeFromCart(service.id)} className="text-gray-400 hover:text-red-500 transition-colors">
                      <Trash2 size={20}/>
                    </button>
                  </div>
                </div>))}
            ...
            <div className="md:col-span-1">
              <div className="bg-gray-800 p-8 rounded-sm shadow-lg border-t-4 border-amber-600 sticky top-24">
                ...
                <div className="space-y-5 mb-8">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="date" min={today} value={date} className="w-full pl-10 pr-4 py-3 rounded-sm border border-gray-600 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none bg-gray-700 text-white" onChange={(e) => setDate(e.target.value)}/>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <select className="w-full pl-10 pr-4 py-3 rounded-sm border border-gray-600 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none appearance-none bg-gray-700 text-white" onChange={(e) => setTime(e.target.value)}>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <select className="w-full pl-10 pr-4 py-3 rounded-sm border border-gray-600 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none appearance-none bg-gray-700 text-white" onChange={(e) => setSelectedBarberId(e.target.value)}>
                ...
                <button onClick={handleProceedToCheckout} disabled={!date || !time || isProcessing} className="w-full py-4 bg-gray-900 text-white rounded-sm font-bold uppercase tracking-wider hover:bg-amber-600 hover:text-black disabled:bg-gray-700 disabled:cursor-not-allowed disabled:hover:bg-gray-700 disabled:text-gray-400 transition-all">
        ...
        {step === 2 && (<div className="max-w-2xl mx-auto bg-gray-800 rounded-sm shadow-xl overflow-hidden">
            ...
            <div className="p-8">
              <div className="flex gap-4 mb-8">
                <button onClick={() => setPaymentMethod('online')} className={`flex-1 py-4 rounded-sm border-2 font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all ${paymentMethod === 'online' ? 'border-amber-600 text-amber-600 bg-gray-800' : 'border-gray-600 text-gray-400 hover:border-gray-500 bg-gray-700'}`}>
                  <CreditCard size={20}/> Pay Online
                </button>
                <button onClick={() => setPaymentMethod('salon')} className={`flex-1 py-4 rounded-sm border-2 font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all ${paymentMethod === 'salon' ? 'border-amber-600 text-amber-600 bg-gray-800' : 'border-gray-600 text-gray-400 hover:border-gray-500 bg-gray-700'}`}>
                  <Store size={20}/> Pay in Shop
                </button>
              </div>

              {paymentMethod === 'online' ? (<form onSubmit={handlePayment} className="space-y-6">
                  ...
                  <button type="submit" disabled={isProcessing} className="w-full py-4 bg-amber-600 text-black rounded-sm font-bold uppercase tracking-wider hover:bg-amber-500 transition-all shadow-lg disabled:opacity-70 disabled:cursor-wait">
                    {isProcessing ? 'Processing...' : `Pay $${(total * 1.08).toFixed(2)}`}
                  </button>
                </form>) : (<div className="text-center space-y-8">
                  ...
                  <button onClick={handlePayment} disabled={isProcessing} className="w-full py-4 bg-gray-900 text-white rounded-sm font-bold uppercase tracking-wider hover:bg-amber-600 hover:text-black transition-all shadow-lg">
        ...
        {step === 3 && (<div className="max-w-xl mx-auto bg-gray-800 rounded-sm shadow-2xl overflow-hidden text-center">
            ...
            <div className="p-8">
              ...
              <div className="bg-gray-700 border-2 border-amber-600 rounded-lg p-6 mb-6">
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <button onClick={copyBookingNumber} className="p-2 hover:bg-gray-600 rounded transition-colors" title="Copy booking number">
              ...
              <div className="flex gap-3">
                <Link to="/my-bookings" className="flex-1 px-6 py-3 bg-gray-900 text-white rounded-sm font-bold uppercase tracking-wider hover:bg-amber-600 hover:text-black transition-colors">
                  View My Bookings
                </Link>
                <Link to="/" className="flex-1 px-6 py-3 border-2 border-gray-600 text-gray-300 rounded-sm font-bold uppercase tracking-wider hover:border-gray-500 transition-colors">
                  Return Home
                </Link>
              </div>
            </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalTitle} actions={modalActions}>
        {modalContent}
      </Modal>
    </div>);
};
