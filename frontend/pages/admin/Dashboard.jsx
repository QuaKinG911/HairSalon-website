...
import { Package, MessageSquare, Users, LogOut, Calendar, Scissors } from 'lucide-react';
    ...
    const loadStats = async () => {
        try {
            ...
            setStats({
                ...
                totalMessages: messages.filter((m) => !m.read).length,
            ...
            <header className="bg-gray-800 border-b border-gray-700 shadow-lg sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    ...
                    <div className="flex gap-6 mt-6 border-b border-gray-700 overflow-x-auto">
                        <button onClick={() => setActiveTab('overview')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative whitespace-nowrap ${activeTab === 'overview' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Overview
                            {activeTab === 'overview' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500"></div>}
                        </button>
                        <button onClick={() => setActiveTab('bookings')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative whitespace-nowrap ${activeTab === 'bookings' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Bookings
                            {activeTab === 'bookings' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500"></div>}
                        </button>
                        <button onClick={() => setActiveTab('services')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative whitespace-nowrap ${activeTab === 'services' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Services
                            {activeTab === 'services' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500"></div>}
                        </button>
                        <button onClick={() => setActiveTab('barbers')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative whitespace-nowrap ${activeTab === 'barbers' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Barbers
                            {activeTab === 'barbers' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500"></div>}
                        </button>
                        <button onClick={() => setActiveTab('messages')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative whitespace-nowrap ${activeTab === 'messages' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Messages
                            {stats.totalMessages > 0 && (<span className="ml-2 px-1.5 py-0.5 bg-red-500 text-white text-xs rounded-full">
                                    {stats.totalMessages}
                                </span>)}
                ...
                {activeTab === 'overview' && (<div className="space-y-8">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <button onClick={() => setActiveTab('services')} className="bg-gray-800 rounded-sm border border-gray-700 p-6 hover:border-amber-500/50 hover:bg-gray-750 transition-all text-left group">
                                ...
                                <button onClick={() => setActiveTab('barbers')} className="bg-gray-800 rounded-sm border border-gray-700 p-6 hover:border-amber-500/50 hover:bg-gray-750 transition-all text-left group">
                                ...
                                <button onClick={() => setActiveTab('messages')} className="bg-gray-800 rounded-sm border border-gray-700 p-6 hover:border-amber-500/50 hover:bg-gray-750 transition-all text-left group">
