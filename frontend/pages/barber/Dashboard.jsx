...
import { LogOut, Calendar, Clock, Search, Package, CreditCard, Phone, Mail, CheckCircle, XCircle, Save } from 'lucide-react';
...
const BarberDashboard = () => {
    ...
    const BookingCard = ({ booking, highlight }) => (<div className={`border rounded-sm p-4 transition-all ${highlight ? 'border-amber-500 bg-amber-900/10' : 'border-gray-700 bg-gray-800 hover:border-gray-600'}`}>
    ...
    return (<div className="min-h-screen bg-gray-900 text-gray-100 font-sans">
            <header className="bg-gray-800 border-b border-gray-700 shadow-lg sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 bg-red-900/50 text-red-200 border border-red-800 rounded-sm hover:bg-red-900 transition-colors">
                    ...
                    <div className="flex gap-6 mt-6 border-b border-gray-700">
                        <button onClick={() => setActiveTab('overview')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative ${activeTab === 'overview' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Overview
                            {activeTab === 'overview' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500"></div>}
                        </button>
                        <button onClick={() => setActiveTab('bookings')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative ${activeTab === 'bookings' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Bookings
                            {activeTab === 'bookings' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500"></div>}
                        </button>
                        <button onClick={() => setActiveTab('schedule')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative ${activeTab === 'schedule' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Schedule
                            {activeTab === 'schedule' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500"></div>}
                        </button>
                        <button onClick={() => setActiveTab('messages')} className={`pb-3 px-1 font-bold text-sm uppercase tracking-wider transition-colors relative ${activeTab === 'messages' ? 'text-amber-500' : 'text-gray-400 hover:text-gray-200'}`}>
                            Messages
                            {messages.some(m => !m.read) && (<span className="ml-2 px-1.5 py-0.5 bg-red-500 text-white text-xs rounded-full">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {activeTab === 'overview' && (<div className="space-y-8">
                        ...
                        <div>
                            ...
                            {getTodayBookings().length === 0 ? (<div className="bg-gray-800 rounded-sm border border-gray-700 p-8 text-center">
                                    ...
                                    {getTodayBookings().map(booking => (<BookingCard key={booking.id} booking={booking}/>))}
                                </div>)}
                        </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <button onClick={() => setActiveTab('schedule')} className="bg-gray-800 rounded-sm border border-gray-700 p-6 hover:border-amber-500/50 hover:bg-gray-750 transition-all text-left group">
                                ...
                                <button onClick={() => setActiveTab('bookings')} className="bg-gray-800 rounded-sm border border-gray-700 p-6 hover:border-amber-500/50 hover:bg-gray-750 transition-all text-left group">
                ...
                {/* BOOKINGS TAB */}
                {activeTab === 'bookings' && (<div className="space-y-8">
                        {/* Search */}
                        <div className="bg-gray-800 rounded-sm border border-gray-700 p-6 shadow-lg">
                            ...
                            <form onSubmit={handleSearch} className="flex gap-3">
                                <div className="flex-1 relative">
                                    ...
                                    <input type="text" value={searchNumber} onChange={(e) => setSearchNumber(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="Enter 6-digit booking number" className="w-full pl-10 pr-4 py-3 bg-gray-900 border border-gray-600 rounded-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent text-white placeholder-gray-500" maxLength={6}/>
                                </div>
                                <button type="submit" className="px-6 py-3 bg-amber-600 text-black rounded-sm font-bold hover:bg-amber-500 transition-colors uppercase tracking-wide">
                                    Search
                                </button>
                            </form>

                            {searchError && (<div className="mt-4 p-4 bg-red-900/20 border border-red-800 rounded-sm text-red-400 text-sm">
                                    {searchError}
                                </div>)}
                        ...
                        <div>
                            ...
                            {bookings.length === 0 ? (<p className="text-gray-400">No bookings found.</p>) : (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {/* SCHEDULE TAB */}
                {activeTab === 'schedule' && (<div className="space-y-8">
                        {/* Today's Schedule */}
                        <div className="bg-gray-800 rounded-sm border border-gray-700 p-6 shadow-lg">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="flex gap-3">
                                    {scheduleMessage && (<div className="flex items-center gap-2 text-green-400 font-medium">
                                            <CheckCircle size={20}/>
                                            {scheduleMessage}
                                        </div>)}
                                    <button onClick={() => setIsEditingSchedule(!isEditingSchedule)} className={`px-4 py-2 rounded-sm font-bold transition-colors ${isEditingSchedule
                ? 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                : 'bg-amber-600 text-black hover:bg-amber-500'}`}>
                                        {isEditingSchedule ? 'Cancel' : 'Edit Schedule'}
                                    </button>
                                    {isEditingSchedule && (<button onClick={saveSchedule} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-sm font-bold hover:bg-green-500 transition-colors">
                                            <Save size={18}/>
                                            Save Changes
                                        </button>)}
                                </div>
                            </div>
                            {todaySlot ? (<div className="flex items-center gap-4">
                                    ...
                                    {todaySlot.isActive && (<div className="flex items-center gap-2 text-gray-400">
                                            <Clock size={16}/>
                                            <span>{todaySlot.startTime} - {todaySlot.endTime}</span>
                                        </div>)}
                                </div>) : (<p className="text-gray-500">No schedule set for today</p>)}
                        </div>
                        ...
                        <div className="bg-gray-800 rounded-sm border border-gray-700 shadow-lg overflow-hidden">
                            ...
                            <div className="p-8">
                                <div className="grid gap-4">
                                    {schedule.map((slot) => (<div key={slot.id} className={`border rounded-sm p-6 transition-all ${slot.isActive ? 'border-green-900/50 bg-green-900/10' : 'border-gray-700 bg-gray-800'}`}>
                                            <div className="flex items-center justify-between mb-4">
                                                <div className="flex items-center gap-3">
                                                ...
                                                {isEditingSchedule && (<label className="flex items-center gap-2 cursor-pointer">
                                                        <input type="checkbox" checked={slot.isActive} onChange={(e) => updateSlot(slot.id, { isActive: e.target.checked })} className="w-4 h-4 text-amber-600 rounded focus:ring-amber-500 bg-gray-700 border-gray-600"/>
                                            ...
                                            {slot.isActive && (<div className="grid md:grid-cols-2 gap-4">
                                                    <div>
                                                        <label className="block text-sm font-bold text-gray-400 mb-2">
                                                            Start Time
                                                        </label>
                                                        {isEditingSchedule ? (<input type="time" value={slot.startTime} onChange={(e) => updateSlot(slot.id, { startTime: e.target.value })} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 text-white rounded-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent"/>) : (<div className="flex items-center gap-2 px-4 py-2 bg-gray-700/50 border border-gray-600 rounded-sm">
                                                    ...
                                                    <div>
                                                        ...
                                                        {isEditingSchedule ? (<input type="time" value={slot.endTime} onChange={(e) => updateSlot(slot.id, { endTime: e.target.value })} className="w-full px-4 py-2 bg-gray-700 border border-gray-600 text-white rounded-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent"/>) : (<div className="flex items-center gap-2 px-4 py-2 bg-gray-700/50 border border-gray-600 rounded-sm">
                                                                <Clock size={16} className="text-gray-400"/>
                ...
                {/* MESSAGES TAB */}
                {activeTab === 'messages' && (<Messenger className="h-[600px]"/>)}
            </main>
        </div>);
};
