...
export const isValidBookingNumber = (number) => {
    return /^\d{6}$/.test(number);
};
// Check if booking number exists
export const bookingNumberExists = (number) => {
    const bookings = localStorage.getItem('bookings');
    if (!bookings)
        return false;
    const bookingList = JSON.parse(bookings);
    return bookingList.some((b) => b.bookingNumber === number);
};
// Generate unique booking number (ensures uniqueness)
export const generateUniqueBookingNumber = () => {
