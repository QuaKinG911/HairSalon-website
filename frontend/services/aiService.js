...
export class AIService {
    ...
    detectFace(pixels, width, height) {
    ...
    analyzeFaceShape(pixels, width, faceBox) {
        ...
        if (Math.abs(faceWidth - faceHeight) < faceWidth * 0.2) {
            ...
            if (adjustedJawWidth > cheekWidth * 0.9)
                return 'square';
            return 'round';
        }
        else {
            ...
            if (cheekWidth > foreheadWidth && cheekWidth > adjustedJawWidth) {
                if (adjustedJawWidth < foreheadWidth * 0.7)
                    return 'diamond';
                return 'oval';
            }
            // Heart shape requires forehead to be significantly wider
            if (foreheadWidth > cheekWidth * 1.05 && foreheadWidth > adjustedJawWidth)
                return 'heart';
            // Default to oval if no strong indicators
            return 'oval';
        }
    }
    getSkinWidthAtY(pixels, width, y, minX, maxX) {
        let start = -1, end = -1;
    ...
    analyzeHair(pixels, width, height, faceBox) {
        ...
        for (let y = hairStartY; y < hairEndY; y += 2) { // Skip every other pixel for speed
            for (let x = hairStartX; x < hairEndX; x += 2) {
                ...
                if (x < width - 1 && y < height - 1) {
                    ...
                    if (Math.abs(brightness - brightnessNext) > 15)
                        colorVarianceScore++;
                }
                pixelCount++;
            }
        ...
        if (curlScore > 0.25)
            hairType = 'coily';
        else if (curlScore > 0.15)
            hairType = 'curly';
        else if (curlScore > 0.08)
            hairType = 'wavy';
        else
            hairType = 'straight';
        // Length estimation
        ...
        if (faceBox.height > faceBox.width * 1.5)
            hairLength = 'long';
        else if (faceBox.height > faceBox.width * 1.25)
            hairLength = 'medium';
        return { hairType, hairLength };
    }
    // --- STEP 4: SKIN ANALYSIS ---
    analyzeSkinTone(pixels, width, faceBox) {
    ...
    async getHairstyleRecommendations(analysis, preferences) {
        ...
        if (filteredStyles.length < 3)
            filteredStyles = HAIRSTYLES;
        if (filteredBeards.length < 3)
            filteredBeards = BEARDS;
        ...
        return [...topStyles, ...topBeards, combo];
    }
    generateDescription(style, analysis) {
        return `${style.description} Perfect for your ${analysis.faceShape} face and ${analysis.hairType} hair.`;
    }
    generateBeardDescription(beard, analysis) {
